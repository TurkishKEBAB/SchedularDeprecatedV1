"""
Data models for the course scheduler application.

This module defines the core classes that represent courses, schedules, and programs.
Each class uses proper type hints and follows object-oriented design principles.
"""
...
class Course:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Course':
        ...
        return cls(
            ...
            name=data.get("course_name", data.get("name", "Unknown Course")),  # Fixed: handle both keys
            ects=data.get("credit", data.get("ects", 0)),  # Fixed: handle both keys
            course_type=data["course_type"],
            schedule=data["schedule"],
            teacher=data["teacher"],
            has_lecture=data["has_lecture"],
...
class Schedule:
    ...
    def conflict_count(self) -> int:
        ...
        """
        # Çakışmaların daha detaylı incelenmesi için çakışan dersleri de kaydedelim
        slot_courses: Dict[TimeSlot, List[Course]] = defaultdict(list)
        conflict_slots = set()
        conflict_details = []

        # Her dersin zaman dilimini kontrol et
        for course in self.courses:
            for slot in course.schedule:
                slot_courses[slot].append(course)
                # Eğer bu zaman diliminde birden fazla ders varsa, çakışma var demektir
                if len(slot_courses[slot]) > 1:
                    conflict_slots.add(slot)
                    # Bu çakışma daha önce kaydedilmediyse ekle
                    conflict_str = f"Conflict at {slot[0]}{slot[1]}: {', '.join(c.code for c in slot_courses[slot])}"
                    if conflict_str not in conflict_details:
                        conflict_details.append(conflict_str)

        # Detaylı çakışma bilgilerini loglayalım
        if conflict_slots:
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"Found {len(conflict_slots)} conflicts in schedule:")
            for detail in conflict_details:
                logger.warning(detail)

        # Çakışma sayısını dön (her çakışan zaman dilimi 1 çakışma olarak sayılır)
...
def get_courses_by_teacher(courses: List[Course], teacher: str) -> List[Course]:
    ...
    """
    return [c for c in courses if c.teacher == teacher]
