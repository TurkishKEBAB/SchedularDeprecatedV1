...
class AnnealingOptimizer:
    ...
    def __init__(self,
        ...
        self.scheduler_prefs = scheduler_prefs
    ...
    def optimize(self,
        ...
        def fitness(sched: List[Course], total: int) -> float:
            ...
            if self.scheduler_prefs:
                # Apply advanced scoring (lower score = better)
    ...
    def score_schedule(self, schedule: Schedule) -> float:
        """
        Calculate the score of a schedule based on advanced criteria.

        Args:
            schedule: Schedule to evaluate

        Returns:
            Score of the schedule (higher is better)
        """
        score = 0.0

        # Example criteria (to be replaced with actual logic):
        if self.scheduler_prefs:
            if self.scheduler_prefs.max_weekly_slots < 60:  # Fixed: use max_weekly_slots instead of min_weekly_hours
                score += meets_weekly_hours_constraint(schedule, self.scheduler_prefs.max_weekly_slots)
            if self.scheduler_prefs.max_daily_slots:  # Fixed: use max_daily_slots instead of max_daily_hours
                score += meets_daily_hours_constraint(schedule, self.scheduler_prefs.max_daily_slots)
            if self.scheduler_prefs.desired_free_days:  # Fixed: use desired_free_days instead of free_days
                score += meets_free_day_constraint(schedule, self.scheduler_prefs.desired_free_days, strict=True)  # Fixed: add missing strict parameter
